function result = KernelFunc(x1, x2, name,param)
    %  核函数计算,name指示不同的核函数类型
    %  函数会根据name和超参数param计算出向量x1和向量x2之间的协方差
    if (name == "eqk" && length(param)==2)  %平方指数核函数
    ...
    if (name=="pk" && length(param)==3)  %L1范数周期核函数
        Distance=abs(x1'-x2);
    ...
    if (name=="lpk" && length(param)==4)  %混合周期核函数
        Distance1=abs(x1'-x2);
        Distance2=(x1.^2)'+(x2.^2)-2*x1'*x2;
        pk=Distance1*pi/param(3);
        pk=2*sin(pk)^2/(param(2)^2);
        eqk=1/(2*param(1)^2*Distance2);
        result=param(4)*exp(-pk-eqk);
    else
        disp("参数个数与核函数不匹配!");
        return;        
    end
